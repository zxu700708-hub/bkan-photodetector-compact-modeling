...
def create_dataset(f, 
    ...
    -----
        f : function
            用于生成数据的真实数学公式（符号函数）。例如: lambda x: torch.sin(x)
        ...
        f_mode : str
            公式 f 期望的输入格式。
            'col' (默认): 输入矩阵的列代表变量 (N, n_var)。这是 PyTorch 的标准格式。
            'row': 输入矩阵的行代表变量 (n_var, N)。
        
        ranges : list 或 np.array
            输入变量的取值范围。
            形状可以是 (2,)，如 [-1, 1]，表示所有变量都在此范围内。
            也可以是 (n_var, 2)，分别为每个变量指定不同的范围。
            默认为 [-1, 1]。
        ...
        normalize_input : bool
            是否对输入数据进行归一化（减均值除标准差）。默认为 False。
        
        normalize_label : bool
            是否对标签（输出）数据进行归一化。默认为 False。
        
        device : str
            数据存储的设备，如 'cpu' 或 'cuda'。默认为 'cpu'。
        
        seed : int
            随机种子，用于控制随机数生成，确保结果可复现。默认为 0。
        
    返回 (Returns):
    --------
        dataset : dict
            包含训练和测试数据的字典。
            键包括: 'train_input', 'train_label', 'test_input', 'test_label'
    '''
    ...
    # 处理输入变量的取值范围 ranges
    # 如果只给了一个范围（如 [-1, 1]），则应用到所有变量上，否则每个变量都有独立的范围
    if len(np.array(ranges).shape) == 1:
        ranges = np.array(ranges * n_var).reshape(n_var,2)
    else:
        ranges = np.array(ranges)
        
    # 初始化训练集和测试集的输入 Tensor (全零矩阵)
    train_input = torch.zeros(train_num, n_var)
    test_input = torch.zeros(test_num, n_var)

    # 为每个变量在指定范围内生成均匀分布的随机数
    for i in range(n_var):
        # torch.rand 生成 [0, 1) 的随机数，通过线性变换映射到 [min, max]
        train_input[:,i] = torch.rand(train_num,)*(ranges[i,1]-ranges[i,0])+ranges[i,0]
        test_input[:,i] = torch.rand(test_num,)*(ranges[i,1]-ranges[i,0])+ranges[i,0]
                
    # 根据提供的公式 f 计算标签 (Label)
    if f_mode == 'col':
        # 默认模式：按列处理输入，直接将 Tensor 传给 f
        train_label = f(train_input)
        test_label = f(test_input)
    elif f_mode == 'row':
        # 兼容模式：如果公式 f 期望输入是转置后的，则先转置
        train_label = f(train_input.T)
        test_label = f(test_input.T)
    else:
        print(f'f_mode {f_mode} not recognized')
        
    # 如果输出是一维的 (N,)，将其升维成二维 (N, 1)，以符合 PyTorch 的标准
    ...
    # 将处理好的数据打包成字典
    dataset = {}
    # .to(device) 将 Tensor 移动到指定的设备 (CPU 或 GPU)
