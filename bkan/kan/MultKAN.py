...
class MultKAN(nn.Module):
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True, random=False, log_history=True):
        '''
        将 (l,i,j) 处的激活函数固定为符号函数 (由 fun_name 指定)
        ...
        -----
            ...
            fit_params_bool : bool
                是否通过拟合获取仿射参数 (True) 还是使用默认值 (False)
            a_range : tuple
                参数 a 的扫描范围
            b_range : tuple
                参数 b 的扫描范围
    ...
    def unfix_symbolic(self, l, i, j, log_history=True):
        '''
        取消固定 (l,i,j) 处的激活函数 (恢复为样条函数模式)
        '''
        # 将模式设置回 "n" (numeric，数值/样条模式)
    ...
    def reg(self, reg_metric, lamb_l1, lamb_entropy, lamb_coef, lamb_coefdiff):
        ...
        for i in range(len(acts_scale)):
            ...
            # L1 正则化: 鼓励激活值稀疏 (sum of absolute values, though vec is likely already positive/magnitude)
            l1 = torch.sum(vec)
            
            # 熵正则化 (Entropy): 鼓励连接权重的分布“二值化”（要么强要么弱），减少模棱两可的连接
            # 计算行和列的归一化概率
        ...
        # 对样条系数 (Spline Coefficients) 进行正则化
        # regularize coefficient to encourage spline to be zero
        for i in range(len(self.act_fun)):
            # 系数 L1 正则化: 限制系数大小，防止过拟合
            coeff_l1 = torch.sum(torch.mean(torch.abs(self.act_fun[i].coef), dim=1))
            # 系数平滑正则化 (Smoothness): 限制相邻系数的差异，鼓励曲线平滑
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1.,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
        ...
        -----
            dataset : dic
                包含了训练和测试数据: dataset['train_input'], dataset['train_label'], dataset['test_input'], dataset['test_label']
            opt : str
                优化器选择: "LBFGS" 或 "Adam"
            steps : int
                训练步数
            log : int
                日志记录频率
            lamb : float
                总体正则化强度 (如果 > 0，将开启稀疏化训练，为剪枝做准备)
            lamb_l1 : float
                L1 正则化强度 (鼓励权重趋向于 0，实现稀疏性)
            lamb_entropy : float
                熵正则化强度 (鼓励连接权重呈现二值化，即要么强要么无，减少模棱两可的连接)
            lamb_coef : float
                系数大小的惩罚强度
            lamb_coefdiff : float
                相邻系数差异的惩罚强度 (鼓励函数的平滑性)
            update_grid : bool
                是否在训练过程中自动更新网格 (自适应网格)
            grid_update_num : int
                停止更新网格前的总更新次数
            ...
            y_th : float
                奇异点阈值 (超过此值的输出会被软化处理)
            reg_metric : str
                正则化度量标准。可选值: {'edge_forward_spline_n', 'edge_forward_spline_u', 'edge_forward_sum', 'edge_backward', 'node_backward'}
        ...
        --------
            results : dic
                results['train_loss']: 训练损失记录 (1D array)
                results['test_loss']: 测试损失记录 (1D array)
                results['reg']: 正则化项的值记录
                其他在 metrics 中指定的指标
        ...
        # 检查参数冲突：如果开启了正则化 (lamb > 0)，必须确保激活值被保存 (save_act=True)，
        # 因为正则化计算需要用到神经元的激活情况。
        if lamb > 0. and not self.save_act:
            print('setting lamb=0. If you want to set lamb > 0, set self.save_act=True')
        
        # 在 fit 过程中根据 lamb 的设置暂时禁用符号化部分，专注于数值优化
        ...
        if opt == "Adam":
            ...
            # LBFGS 是一种拟牛顿法，通常在小批量数据和科学计算中比 Adam 收敛更快、精度更高
    ...
    def suggest_symbolic(self, l, i, j, a_range=(-10, 10), b_range=(-10, 10), lib=None, topk=5, verbose=True, r2_loss_fun=lambda x: np.log2(1+1e-5-x), c_loss_fun=lambda x: x, weight_simple = 0.8):
        ...
        if lib == None:
            symbolic_libjnihbbvv  = SYMBOLIC_LIB
        else:
            symbolic_lib = {}
            for item in lib:
    ...
    def symbolic_formula(self, var=None, normalizer=None, output_normalizer=None, simplify=True):
        '''
        ...
        symbolic_acts = [] # 存储每一层的符号化激活输出
        symbolic_acts_premult = [] # 存储乘法节点前的中间结果
        x = [] # 当前层的输入变量（符号形式）

        # 辅助函数：对 sympy 表达式中的浮点数进行四舍五入
        ...
        # 2. 应用输入归一化 (Input Normalization)
        # 如果提供了 normalizer，则对输入 x 进行 (x - mean) / std 变换
        if normalizer != None:
            mean = normalizer[0]
            std = normalizer[1]
            x = [(x[i] - mean[i]) / std[i] for i in range(len(x))]

        symbolic_acts.append(x) # 将处理后的输入层作为第 0 层的激活值

        # 3. 逐层构建公式 (Layer-wise Construction)
        for l in range(len(self.width_in) - 1):
            num_sum = self.width[l + 1][0] # 当前层的加法节点数量
            num_mult = self.width[l + 1][1] # 当前层的乘法节点数量
            y = [] # 存储当前层的输出（未经过节点缩放和偏置）

            # 3.1 处理从上一层到当前层的连接 (Edges)
            # j 是当前层的第 j 个神经元（目标）
            for j in range(self.width_out[l + 1]):
                yj = 0.
                # i 是上一层的第 i 个神经元（源）
                for i in range(self.width_in[l]):
                    # 获取该连接上的仿射变换参数 (a, b, c, d)
                    # 对应的公式是 c * f(a * x + b) + d
                    a, b, c, d = [
                        float(v.detach().cpu())
                        for v in self.symbolic_fun[l].affine[j, i]
                    ]
                    # 获取该连接上已经确定的符号函数 f (如 sin, x^2)
                ...
                # 应用子节点（Subnode）层面的缩放和偏置
                # yj = subnode_scale * yj + subnode_bias
                yj = float(self.subnode_scale[l][j].detach().cpu()) * yj + float(self.subnode_bias[l][j].detach().cpu())
            ...
            symbolic_acts_premult.append(y) # 保存乘法操作前的中间结果
                  
            # 3.2 处理乘法节点 (Multiplication Nodes)
            # 如果存在乘法节点，需要将之前计算出的 y 中的某些部分乘起来
            mult = []
            for k in range(num_mult):
                # 获取该乘法节点的元数 (arity)，即乘几个数
                if isinstance(self.mult_arity, int):
                    mult_arity = self.mult_arity
                else:
                ...
                # 执行乘法操作
                # 假设加法节点有 num_sum 个，那么乘法节点的输入从 y[num_sum] 开始
            ...
            # 3.3 组合加法节点和乘法节点的输出
            # y 的前 num_sum 个是加法结果，后面拼接乘法结果
            y = y[:num_sum] + mult
            
            # 3.4 应用节点（Node）层面的缩放和偏置
            # y = node_scale * y + node_bias
            for j in range(self.width_in[l+1]):
                y[j] = float(self.node_scale[l][j].detach().cpu()) * y[j] + float(self.node_bias[l][j].detach().cpu())
            
            x = y # 将当前层的输出 y 作为下一层的输入 x
            symbolic_acts.append(x)

        # 4. 应用输出反归一化 (Output Denormalization)
        # 如果提供了 output_normalizer，将输出变换回原始尺度：y = y * std + mean
        if output_normalizer != None:
            ...
            output_layer = [(output_layer[i] * float(stds[i]) + float(means[i])) for i in range(len(output_layer))]
