...
def _condition(device, voltage: np.ndarray | list[float]):
...
def prepare(bundle: Path) -> int:
    if sha256(MODEL) != MODEL_SHA256:
        raise RuntimeError("Canonical Verilog-A bytes no longer match the validated hash")
    ...
    for count in MULTI_COUNTS:
        ...
        (bundle / name).write_text(_instance_deck(count), encoding="utf-8", newline="\n")
    rows = []
...
def parse_number(text: str) -> complex | None:
...
def verify(run_root: Path, output: Path) -> int:
    ...
    source_binding = {
        "passed": source_hash == MODEL_SHA256 == manifest.get("model_sha256"),
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        return prepare(args.bundle.resolve())
